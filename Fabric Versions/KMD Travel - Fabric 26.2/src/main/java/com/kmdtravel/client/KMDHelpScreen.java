...
public class KMDHelpScreen extends KMDScreen {
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.text(font, "KMD Travel Manual", panelX + 10, panelY + 8, 0xFFFFE6B0, false);
    ...
    private void drawTabs(GuiGraphicsExtractor graphics, int mouseX, int mouseY, int x, int y, int maxWidth) {
        int tabX = x;
        int tabY = y;
        for (int i = 0; i < PAGES.length; i++) {
            ...
            graphics.text(font, PAGES[i].title(), tabX + 7, tabY + 4, 0xFFFFE6B0, false);
            tabX += tabW + 4;
        }
    }
    ...
    private void renderPage(GuiGraphicsExtractor graphics, int x, int y, int width, int height) {
    ...
    private int drawSection(GuiGraphicsExtractor graphics, String header, String text, int x, int y, int width) {
        graphics.text(font, header, x, y, 0xFFFFD36A, false);
        int lineY = y + 13;
        for (FormattedCharSequence line : font.split(Component.literal(text), width - 12)) {
            graphics.text(font, line, x + 10, lineY, 0xFFFFE6B0, false);
            lineY += 11;
        }
        return lineY;
    }

    private int drawExample(GuiGraphicsExtractor graphics, String header, String text, int x, int y, int width) {
        graphics.text(font, header, x, y, 0xFFFFD36A, false);
        ...
        for (FormattedCharSequence line : lines) {
            graphics.text(font, line, x + 16, lineY, 0xFFBFEFFF, false);
            lineY += 11;
        }
        return boxBottom;
    }
