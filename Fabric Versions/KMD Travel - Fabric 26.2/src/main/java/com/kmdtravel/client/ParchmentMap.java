...
public final class ParchmentMap {
    ...
    public static void drawFrame(GuiGraphicsExtractor graphics, Font font, Component title, int width, int height) {
        drawFrame(graphics, font, title, width, height, true);
    }

    public static void drawFrame(GuiGraphicsExtractor graphics, Font font, Component title, int width, int height, boolean listOpen) {
        ...
        graphics.text(font, title, titleX, 15, 0xFFF5D99C, false);
    }

    public static void drawMap(GuiGraphicsExtractor graphics, Font font, Bounds bounds, long seed, Identifier dimension, List<OpenTravelScreenPacket.Entry> locations,
    ...
    public static void drawMapWithoutLabels(GuiGraphicsExtractor graphics, Font font, Bounds bounds, long seed, Identifier dimension, List<OpenTravelScreenPacket.Entry> locations,
                                            UUID sourceId, Progress progress, double panX, double panY, double zoom, double mouseX, double mouseY) {
        drawMap(graphics, font, bounds, seed, dimension, locations, sourceId, progress, panX, panY, zoom, mouseX, mouseY, false, false, false);
    }

    private static void drawMap(GuiGraphicsExtractor graphics, Font font, Bounds bounds, long seed, Identifier dimension, List<OpenTravelScreenPacket.Entry> locations,
        ...
        if (locations.isEmpty()) {
            ...
            graphics.centeredText(font, Component.translatable("screen.kmdtravel.no_locations"), bounds.x() + bounds.w() / 2, bounds.y() + bounds.h() / 2, 0xFF5A321A);
            return;
        }
        ...
        if (progress != null && centerProgressMarker) {
            ...
            graphics.horizontalLine(lineStartX, lineEndX, lineY, 0xAA7D1B1B);
            graphics.verticalLine(lineX, lineStartY, lineEndY, 0xAA7D1B1B);
        ...
        for (OpenTravelScreenPacket.Entry location : locations) {
            ...
            drawReadableString(graphics, font, location.name(), px + 8, py - 5, 0xFF2D2012);
            if (current) {
                graphics.centeredText(font, Component.translatable("screen.kmdtravel.you_are_here"), px, py + 10, 0xFF6D2716);
            } else if (hot) {
                Component ambush = Component.translatable("screen.kmdtravel.ambush_chance", location.ambushChance());
                drawReadableString(graphics, font, ambush.getString(), px - font.width(ambush) / 2, py + 10, 0xFF2D2012);
            }
        }
        graphics.disableScissor();
    }

    private static void drawShieldPin(GuiGraphicsExtractor graphics, int x, int y, int color, int pattern, boolean highlighted) {
        ...
        if ((pattern & 1) != 0) {
            graphics.verticalLine(x, y - 6, y + 3, accent);
        }
        if ((pattern & 2) != 0) {
            graphics.horizontalLine(x - 3, x + 3, y - 3, accent);
        }
        if ((pattern & 4) != 0) {
    ...
    public static void drawLocationList(GuiGraphicsExtractor graphics, Font font, Bounds bounds, List<OpenTravelScreenPacket.Entry> locations,
        ...
        if (locations.isEmpty()) {
            drawCenteredPlain(graphics, font, Component.translatable("screen.kmdtravel.no_locations"), bounds.x() + bounds.w() / 2, bounds.y() + 32, 0xFF5A321A);
            return;
        }
        ...
        for (int index = first; index < Math.min(locations.size(), first + visible); index++) {
            ...
            drawCenteredPlain(graphics, font, name, bounds.x() + bounds.w() / 2, y, 0xFF2D2012);
        }
    }

    public static void drawLocationToggle(GuiGraphicsExtractor graphics, Font font, Bounds bounds, boolean open) {
        ...
        graphics.centeredText(font, open ? "<" : ">", bounds.x() + bounds.w() / 2, bounds.y() + 7, 0xFF3A2111);
    }
    ...
    public static void drawMapFast(GuiGraphicsExtractor graphics, Font font, Bounds bounds, long seed, Identifier dimension, List<OpenTravelScreenPacket.Entry> locations,
                                   UUID sourceId, Progress progress, double panX, double panY, double zoom, double mouseX, double mouseY) {
        drawMap(graphics, font, bounds, seed, dimension, locations, sourceId, progress, panX, panY, zoom, mouseX, mouseY, false, true, true);
    }

    public static void drawMapFastWithoutLabels(GuiGraphicsExtractor graphics, Font font, Bounds bounds, long seed, Identifier dimension, List<OpenTravelScreenPacket.Entry> locations,
                                                UUID sourceId, Progress progress, double panX, double panY, double zoom, double mouseX, double mouseY) {
        drawMap(graphics, font, bounds, seed, dimension, locations, sourceId, progress, panX, panY, zoom, mouseX, mouseY, false, false, true);
    }

    private static void drawTerrainMap(GuiGraphicsExtractor graphics, Bounds bounds, Scale scale, long seed, Identifier dimension, boolean allowRebuild) {
        ...
        for (int textureZ = minTextureZ; textureZ <= maxTextureZ; textureZ++) {
            for (int textureX = minTextureX; textureX <= maxTextureX; textureX++) {
                ...
                if (texture == null) {
                    ...
                    graphics.blit(RenderPipelines.GUI_TEXTURED, texture.location, drawX, drawY, 0.0F, 0.0F, drawW, drawH, TILE_TEXTURE_SIZE, TILE_TEXTURE_SIZE, TILE_TEXTURE_SIZE, TILE_TEXTURE_SIZE);
                }
            }
        }
    }
    ...
    private static TerrainTile buildTerrainTile(TerrainTileKey key) {
        ...
        for (int y = 0; y < TILE_TEXTURE_SIZE; y++) {
            for (int x = 0; x < TILE_TEXTURE_SIZE; x++) {
                ...
                image.setPixelABGR(x, y, toNativeRgba(cachedTerrainColor(key.seed(), key.dimension(), worldX, worldZ)));
            }
        }
    ...
    private static void drawParchmentDetails(GuiGraphicsExtractor graphics, Bounds bounds) {
    }

    private static void drawBookFrame(GuiGraphicsExtractor graphics, Bounds bounds) {
    ...
    private static void drawBookPanel(GuiGraphicsExtractor graphics, Bounds bounds) {
        drawParchmentBox(graphics, bounds.x() - 10, bounds.y() - 12, bounds.w() + 20, bounds.h() + 24, false);
    }
    private static void drawParchmentBox(GuiGraphicsExtractor graphics, int x, int y, int width, int height, boolean heavy) {
        int right = x + width;
        int bottom = y + height;
    ...
    private static void drawPlayerHead(GuiGraphicsExtractor graphics, int x, int y) {
    ...
    private static void drawCenteredPlain(GuiGraphicsExtractor graphics, Font font, Component text, int centerX, int y, int color) {
        graphics.text(font, text, centerX - font.width(text) / 2, y, color, false);
    }

    private static void drawCenteredPlain(GuiGraphicsExtractor graphics, Font font, String text, int centerX, int y, int color) {
        graphics.text(font, text, centerX - font.width(text) / 2, y, color, false);
    }

    private static void drawReadableString(GuiGraphicsExtractor graphics, Font font, String text, int x, int y, int color) {
