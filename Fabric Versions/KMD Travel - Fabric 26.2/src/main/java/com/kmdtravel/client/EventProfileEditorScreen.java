...
public class EventProfileEditorScreen extends KMDScreen {
    ...
    public EventProfileEditorScreen(OpenEventProfileEditorPacket packet) {
        ...
        ListTag profilesTag = packet.profiles().getListOrEmpty("Profiles");
        for (int i = 0; i < profilesTag.size(); i++) {
            profilesTag.getCompound(i).ifPresent(profileTag -> profiles.add(EventProfile.load(profileTag)));
        }
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.centeredText(font, titleForView(), width / 2, 10, 0xFFE8C878);
        if (helpOpen) {
    ...
    private void renderProfiles(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        int left = width / 2 - 150;
        int top = 58;
        for (int i = scroll; i < profiles.size() && i < scroll + visibleRows(top); i++) {
            ...
            graphics.text(font, profile.name() + "  [" + profile.id() + "]  " + profile.events().size() + " events", left + 5, y + 6, 0xFFFFE6B0, false);
        }
    }

    private void renderEvents(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        if (profiles.isEmpty()) {
            return;
        }
        ...
        int top = 76;
        graphics.text(font, "Name", left + 5, top - 12, 0xFFE8C878, false);
        graphics.text(font, "Status", left + 230, top - 12, 0xFFE8C878, false);
        graphics.text(font, "Biome", left + 315, top - 12, 0xFFE8C878, false);
        graphics.text(font, "Dimension", left + 510, top - 12, 0xFFE8C878, false);
        graphics.text(font, "Delete", left + rowWidth - 70, top - 12, 0xFFE8C878, false);
        ...
        for (int i = scroll; i < events.size() && i < scroll + visibleRows(top); i++) {
            ...
            graphics.text(font, ellipsize(event.title(), 210), left + 5, y + 6, 0xFFFFE6B0, false);
            graphics.text(font, event.enabled() ? "On" : "Off", left + 230, y + 6, event.enabled() ? 0xFF88FF88 : 0xFFFF8888, false);
            graphics.text(font, ellipsize(event.biome().isBlank() ? "Global" : event.biome(), 180), left + 315, y + 6, 0xFFFFE6B0, false);
            graphics.text(font, ellipsize(event.dimension().isBlank() ? "Global" : event.dimension(), 145), left + 510, y + 6, 0xFFFFE6B0, false);
            graphics.fill(left + rowWidth - 70, y + 2, left + rowWidth - 6, y + 18, 0xFF5A2020);
            graphics.text(font, "Delete", left + rowWidth - 58, y + 6, 0xFFFFB0B0, false);
        }
    }

    private void renderEventEditor(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        int left = editorLeft();
        ...
        graphics.text(font, "Mobs added to this event", left + 6, mobsTop + 5, 0xFFE8C878, false);
        ...
        for (int row = 0; row < Math.min(visibleMobRows, editMobs.size() - addedMobScroll); row++) {
            ...
            graphics.text(font, "- " + mobDisplay(editMobs.get(i)), left + 12, rowY, 0xFFFFE6B0, false);
        }
        if (editMobs.isEmpty()) {
            graphics.text(font, "No mobs added yet.", left + 12, mobsTop + 20, 0xFFBFAE86, false);
        }
        ...
        graphics.text(font, "Command list", commandLeft + 6, cmdY + 5, 0xFFE8C878, false);
        ...
        for (int row = 0; row < Math.min(visibleCommandRows, editCommands.size() - commandListScroll); row++) {
            ...
            graphics.text(font, ellipsize("delay " + step.delaySeconds() + "s  |  " + step.command(), commandWidth - 56), commandLeft + 12, rowY, 0xFFFFE6B0, false);
        }
        if (editCommands.isEmpty()) {
            graphics.text(font, "No commands added yet.", commandLeft + 12, cmdY + 22, 0xFFBFAE86, false);
        }
    ...
    private void drawLabel(GuiGraphicsExtractor graphics, String text, int x, int y) {
        graphics.text(font, text, x, y, 0xFFE8C878, false);
    }

    private void renderHelpOverlay(GuiGraphicsExtractor graphics) {
        int panelX = 20;
        int panelY = 54;
        int panelW = width - 40;
        ...
        graphics.text(font, "KMD Event Editor Help", panelX + 8, panelY + 6, 0xFFFFE6B0, false);
        ...
        for (int i = 0; i < tabs.length; i++) {
            ...
            graphics.text(font, tabs[i], tabX + 7, tabY + 4, 0xFFFFE6B0, false);
            tabX += tabW + 4;
        }
        int x = panelX + 14;
        int y = panelY + 58;
    ...
    private int helpSection(GuiGraphicsExtractor graphics, String header, String text, int x, int y, int maxWidth) {
        graphics.text(font, header, x, y, 0xFFFFD36A, false);
        drawWrapped(graphics, text, x + 10, y + 13, maxWidth - 10, 0xFFFFE6B0);
        return y + 13 + wrappedHeight(text, maxWidth - 10) + 12;
    }

    private void helpExample(GuiGraphicsExtractor graphics, String header, String text, int x, int y, int maxWidth) {
        graphics.text(font, header, x, y, 0xFFFFD36A, false);
    ...
    private void renderMobDropdown(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        int left = editorLeft();
        int top = 254;
        ...
        for (int i = 0; i < count; i++) {
            ...
            graphics.text(font, mob, left + 6, y, 0xFFFFE6B0, false);
        }
    }

    private void renderCommandSuggestions(GuiGraphicsExtractor graphics, int x, int y) {
        String[] suggestions = {
        ...
        if (row == 0) {
            graphics.text(font, "No matching helper yet.", x, y, 0xFFBFAE86, false);
        }
    }

    private void renderStringDropdown(GuiGraphicsExtractor graphics, int left, int top, int dropdownWidth, List<String> options, int dropdownScroll) {
        graphics.nextStratum();
        ...
        for (int i = 0; i < count; i++) {
            ...
            graphics.text(font, value, left + 6, top + 4 + i * 14, 0xFFFFE6B0, false);
        }
        if (options.isEmpty()) {
            graphics.text(font, "Select dimension first", left + 6, top + 6, 0xFFBFAE86, false);
        }
    }

    private void renderInlineCommandSuggestions(GuiGraphicsExtractor graphics, int x, int y) {
        if (commandInput == null) {
            return;
        }
        ...
        for (String suggestion : suggestions) {
            ...
            graphics.text(font, ellipsize(suggestion, 620), x + 6, y + row * 11, 0x66FFE6B0, false);
            row++;
            if (row >= 4) {
                break;
            }
    ...
    private void drawWrapped(GuiGraphicsExtractor graphics, String text, int x, int y, int maxWidth, int color) {
        int lineY = y;
        StringBuilder line = new StringBuilder();
        for (String word : text.split(" ")) {
            while (font.width(word) > maxWidth && word.length() > 4) {
                ...
                int cut = Math.max(4, Math.min(word.length(), word.length() * maxWidth / Math.max(1, font.width(word))));
                graphics.text(font, word.substring(0, cut), x, lineY, color, false);
                word = word.substring(cut);
                lineY += 11;
            }
            String next = line.isEmpty() ? word : line + " " + word;
