...
public final class ClientTravelScreens {
    ...
    public static void open(OpenTravelScreenPacket packet) {
        ...
        Minecraft.getInstance().gui.setScreen(new FastTravelMapScreen(packet));
    }

    public static void openTravelProgress(BeginTravelPacket packet) {
        activePrompt = null;
        ClientMapCache.useMap(packet.worldSeed(), packet.dimension());
        Minecraft.getInstance().gui.setScreen(new TravelProgressScreen(packet));
    }

    public static void closeTravelProgress() {
        activePrompt = null;
        Minecraft minecraft = Minecraft.getInstance();
        if (minecraft.gui.screen() instanceof TravelProgressScreen) {
            minecraft.gui.setScreen(null);
        }
    }

    public static void openRenamePost(OpenRenamePostPacket packet) {
        Minecraft.getInstance().gui.setScreen(new RenamePostScreen(packet));
    }

    public static void openEventProfileEditor(OpenEventProfileEditorPacket packet) {
        Minecraft.getInstance().gui.setScreen(new EventProfileEditorScreen(packet));
    }

    public static void openHelp() {
        Minecraft.getInstance().gui.setScreen(new KMDHelpScreen());
    }

    public static void showEventPrompt(TravelEventPromptPacket prompt) {
        activePrompt = prompt;
        Minecraft minecraft = Minecraft.getInstance();
        if (minecraft.gui.screen() instanceof TravelProgressScreen progressScreen) {
