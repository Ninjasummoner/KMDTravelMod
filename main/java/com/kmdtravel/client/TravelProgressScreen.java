...
public class TravelProgressScreen extends KMDScreen {
    ...
    private void drawPrompt(GuiGraphicsExtractor graphics, ParchmentMap.Bounds bounds) {
    ...
    private void drawCenteredShadow(GuiGraphicsExtractor graphics, Component component, int centerX, int y, int color) {
        graphics.text(font, component, centerX - font.width(component) / 2, y, color, true);
    }

    private int drawWrappedCentered(GuiGraphicsExtractor graphics, Component component, int centerX, int y, int maxWidth, int color, int maxLines) {
        List<FormattedCharSequence> lines = font.split(component, maxWidth);
        int drawn = Math.min(maxLines, lines.size());
        for (int i = 0; i < drawn; i++) {
            FormattedCharSequence line = lines.get(i);
            graphics.text(font, line, centerX - font.width(line) / 2, y + i * 12, color, true);
        }
        return y + drawn * 12;
    }
