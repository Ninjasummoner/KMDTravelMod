...
public class FastTravelMapScreen extends Screen {
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double delta) {
        ...
        if (locationsOpen && listBounds.contains(mouseX, mouseY)) {
            ...
            locationScroll = Math.max(0, Math.min(Math.max(0, locations.size() - visible), locationScroll - (int) Math.signum(delta)));
            return true;
        }
        ...
        if (bounds.contains(mouseX, mouseY)) {
            double oldZoom = zoom;
            zoom = ParchmentMap.clampZoom(zoom * (delta > 0 ? 1.18D : 0.85D));
