...
public final class HeldTravelMapRenderer {
    ...
    private static ResourceLocation markerDimension;
    ...
    public static void setMarkers(ResourceLocation dimension, List<HeldMapDataPacket.Marker> updatedMarkers) {
    ...
    public static void render(
        ...
        VertexConsumer mapConsumer = buffers.getBuffer(RenderType.entityCutoutNoCull(textureLocation));
        drawQuad(poseStack, mapConsumer, packedLight, left, top, right, bottom, z, u0, 0.0F, u1, 1.0F);

        if (reveal > 0.08F) {
            renderPlayerHead(poseStack, buffers, packedLight, player, z - 0.012F);
        }
    }
    ...
    private static void updateTexture(AbstractClientPlayer player) {
        ...
        for (int pixelY = 0; pixelY < TEXTURE_HEIGHT; pixelY++) {
            for (int pixelX = 0; pixelX < TEXTURE_WIDTH; pixelX++) {
                ...
                image.setPixelRGBA(pixelX, pixelY, toNativeRgba(color));
            }
        }
    ...
    private static void setMapPixel(int x, int y, int color) {
        ...
        image.setPixelRGBA(x, y, toNativeRgba(color));
    }
    ...
    private static void ensureTexture() {
        ...
        textureLocation = ResourceLocation.fromNamespaceAndPath(KMDTravel.MOD_ID, "held_travel_map");
    ...
    private static void renderPlayerHead(
        ...
        VertexConsumer skinConsumer = buffers.getBuffer(RenderType.entityCutoutNoCull(skin));
    ...
    private static void drawQuad(
        ...
        consumer.vertex(matrix, left, bottom, z).color(255, 255, 255, 255).uv(u0, v1)
                .overlayCoords(OverlayTexture.NO_OVERLAY).uv2(packedLight).normal(0.0F, 0.0F, -1.0F).endVertex();
        consumer.vertex(matrix, right, bottom, z).color(255, 255, 255, 255).uv(u1, v1)
                .overlayCoords(OverlayTexture.NO_OVERLAY).uv2(packedLight).normal(0.0F, 0.0F, -1.0F).endVertex();
        consumer.vertex(matrix, right, top, z).color(255, 255, 255, 255).uv(u1, v0)
                .overlayCoords(OverlayTexture.NO_OVERLAY).uv2(packedLight).normal(0.0F, 0.0F, -1.0F).endVertex();
        consumer.vertex(matrix, left, top, z).color(255, 255, 255, 255).uv(u0, v0)
                .overlayCoords(OverlayTexture.NO_OVERLAY).uv2(packedLight).normal(0.0F, 0.0F, -1.0F).endVertex();
    }
}
