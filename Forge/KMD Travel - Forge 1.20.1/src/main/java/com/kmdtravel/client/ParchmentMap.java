...
public final class ParchmentMap {
    ...
    private static final ResourceLocation PARCHMENT_FRAME = new ResourceLocation("kmdtravel", "textures/gui/parchment_frame.png");
    private static final ResourceLocation PARCHMENT_PANEL = new ResourceLocation("kmdtravel", "textures/gui/parchment_panel.png");
        ...
        for (int textureZ = minTextureZ; textureZ <= maxTextureZ; textureZ++) {
            for (int textureX = minTextureX; textureX <= maxTextureX; textureX++) {
                ...
                if (texture == null) {
                    ...
                    graphics.blit(texture.location, drawX, drawY, drawW, drawH, 0.0F, 0.0F, TILE_TEXTURE_SIZE, TILE_TEXTURE_SIZE, TILE_TEXTURE_SIZE, TILE_TEXTURE_SIZE);
                }
            }
        }
    }
    ...
    private static TerrainTile buildTerrainTile(TerrainTileKey key) {
        ...
        for (int y = 0; y < TILE_TEXTURE_SIZE; y++) {
            for (int x = 0; x < TILE_TEXTURE_SIZE; x++) {
                ...
                image.setPixelRGBA(x, y, toNativeRgba(cachedTerrainColor(key.seed(), key.dimension(), worldX, worldZ)));
            }
        }
        DynamicTexture dynamicTexture = new DynamicTexture(image);
        dynamicTexture.setFilter(false, false);
        ResourceLocation location = Minecraft.getInstance().getTextureManager().register("kmdtravel_map_" + textureSerial++, dynamicTexture);
    ...
    private static void blitPart(GuiGraphics graphics, ResourceLocation texture, int x, int y, int width, int height, int sourceX, int sourceY, int sourceWidth, int sourceHeight) {
        graphics.blit(texture, x, y, width, height, (float) sourceX, (float) sourceY, sourceWidth, sourceHeight, PARCHMENT_TEXTURE_SIZE, PARCHMENT_TEXTURE_SIZE);
    }
