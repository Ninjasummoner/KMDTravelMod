...
public final class KMDCommands {
    ...
    public static void register(RegisterCommandsEvent event) {
        event.getDispatcher().register(Commands.literal("kmd")
        ...
        event.getDispatcher().register(Commands.literal("kmdtravel")
                                ...
                                .then(Commands.argument("player", EntityArgument.player())
                                        .then(Commands.argument("id", StringArgumentType.word())
                                                .executes(context -> {
                                                    ...
                                                    context.getSource().sendSuccess(() -> Component.literal("KMD event profile for " + player.getGameProfile().getName() + " set to: " + profileId), true);
                                                    return 1;
                                                }))))
                        ...
                        .then(Commands.literal("current")
                                ...
                                .then(Commands.argument("player", EntityArgument.player())
                                        .executes(context -> {
                                            EventProfileSavedData data = EventProfileSavedData.get(context.getSource().getLevel());
                                            ServerPlayer player = EntityArgument.getPlayer(context, "player");
                                            context.getSource().sendSuccess(() -> Component.literal("KMD event profile for " + player.getGameProfile().getName() + ": " + data.playerProfile(player)), false);
                                            return 1;
                                        }))))
    ...
    private static int setDouble(com.mojang.brigadier.context.CommandContext<net.minecraft.commands.CommandSourceStack> context,
