...
public final class ParchmentMap {
    ...
    public static void drawFrame(GuiGraphics graphics, Font font, Component title, int width, int height, boolean listOpen) {
        ...
        graphics.drawString(font, title, titleX, 15, 0xFFF5D99C, false);
    }

    public static void drawMap(GuiGraphics graphics, Font font, Bounds bounds, long seed, ResourceLocation dimension, List<OpenTravelScreenPacket.Entry> locations,
    ...
    public static void drawMapWithoutLabels(GuiGraphics graphics, Font font, Bounds bounds, long seed, ResourceLocation dimension, List<OpenTravelScreenPacket.Entry> locations,
                                            UUID sourceId, Progress progress, double panX, double panY, double zoom, double mouseX, double mouseY) {
        drawMap(graphics, font, bounds, seed, dimension, locations, sourceId, progress, panX, panY, zoom, mouseX, mouseY, false, false, false);
    }

    private static void drawMap(GuiGraphics graphics, Font font, Bounds bounds, long seed, ResourceLocation dimension, List<OpenTravelScreenPacket.Entry> locations,
        ...
        ResourceLocation mapDimension = ClientMapCache.mapDimension(dimension);
        graphics.fill(bounds.x(), bounds.y(), bounds.x() + bounds.w(), bounds.y() + bounds.h(), 0xFFE9CB88);

        if (locations.isEmpty()) {
            drawParchmentDetails(graphics, bounds);
            graphics.drawCenteredString(font, Component.translatable("screen.kmdtravel.no_locations"), bounds.x() + bounds.w() / 2, bounds.y() + bounds.h() / 2, 0xFF5A321A);
            return;
        }
        ...
        for (OpenTravelScreenPacket.Entry location : locations) {
            ...
            if (current) {
                graphics.drawCenteredString(font, Component.translatable("screen.kmdtravel.you_are_here"), px, py + 10, 0xFF6D2716);
            } else if (hot) {
    ...
    public static void rememberSamples(long seed, ResourceLocation dimension, List<OpenTravelScreenPacket.MapSample> samples) {
        ResourceLocation mapDimension = ClientMapCache.mapDimension(dimension);
    ...
    public static void rememberSample(long seed, ResourceLocation dimension, int worldX, int worldZ, int color) {
    ...
    public static void drawLocationToggle(GuiGraphics graphics, Font font, Bounds bounds, boolean open) {
        ...
        graphics.drawCenteredString(font, open ? "<" : ">", bounds.x() + bounds.w() / 2, bounds.y() + 7, 0xFF3A2111);
    }
    ...
    public static void drawMapFast(GuiGraphics graphics, Font font, Bounds bounds, long seed, ResourceLocation dimension, List<OpenTravelScreenPacket.Entry> locations,
                                   UUID sourceId, Progress progress, double panX, double panY, double zoom, double mouseX, double mouseY) {
        drawMap(graphics, font, bounds, seed, dimension, locations, sourceId, progress, panX, panY, zoom, mouseX, mouseY, false, true, true);
    }

    public static void drawMapFastWithoutLabels(GuiGraphics graphics, Font font, Bounds bounds, long seed, ResourceLocation dimension, List<OpenTravelScreenPacket.Entry> locations,
                                                UUID sourceId, Progress progress, double panX, double panY, double zoom, double mouseX, double mouseY) {
        drawMap(graphics, font, bounds, seed, dimension, locations, sourceId, progress, panX, panY, zoom, mouseX, mouseY, false, false, true);
    }

    private static void drawTerrainMap(GuiGraphics graphics, Bounds bounds, Scale scale, long seed, ResourceLocation dimension, boolean allowRebuild) {
    ...
    private static TerrainTile buildTerrainTile(TerrainTileKey key) {
        ...
        ResourceLocation location = ResourceLocation.fromNamespaceAndPath("kmdtravel", "map/" + textureSerial++);
        Minecraft.getInstance().getTextureManager().register(location, dynamicTexture);
        dynamicTexture.upload();
        return new TerrainTile(location);
    }

    private static int cachedTerrainColor(long seed, ResourceLocation dimension, int worldX, int worldZ) {
        int tileX = Math.floorDiv(worldX, TILE_SIZE);
        int tileZ = Math.floorDiv(worldZ, TILE_SIZE);
        long key = cacheKey(seed, dimension, tileX, tileZ);
    ...
    private static long cacheKey(long seed, ResourceLocation dimension, int tileX, int tileZ) {
    ...
    private static final class TerrainTile {
        ...
        private TerrainTile(ResourceLocation location) {
            this.location = location;
        }
    }

    private record TerrainTileKey(long seed, ResourceLocation dimension, int textureX, int textureZ) {
    }
